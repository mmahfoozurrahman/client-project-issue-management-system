...
| `IssueController` | Core issue lifecycle: lists/filtering, Kanban, daily activity, create/update/delete, details, pinning, attachments, links, and status changes. | `/issues` -> `Issues/Index`; `/issues/{issue}` -> `Issues/Show`; `/kanban` -> `Issues/Kanban`; `/issues/daily-activity` -> `Issues/DailyActivity`. |
